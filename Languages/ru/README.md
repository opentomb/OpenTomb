...
Вместо этого, разрабатывается все заново, полностью с нуля. В OpenTomb используются исходные коды из других незавершенных проектов, таких как [OpenRaider](http://openraider.sourceforge.net/) и проект VT (см. [icculus.org](https://icculus.org/)), а также некоторый код от Quake Tenebrae.
...
Это правда, что под Windows сейчас без проблем можно запустить TR2-5, и TR1 с применением [DosBox](https://www.dosbox.com/). Однако со временем
...
Верно и то, что для оригинального движка есть патчи улучшающие его: TREP, TRNG и т. д. Преимущество OpenTomb заключается в том, что мы не ограничены оригинальным Binary, что является весомым ограничением, когда дело доходит до новых функций, графических улучшений, модификаций кода и многого другого. Открытый исходный код движка устраняет эти ограничения.

### Особенности ###
* OpenTomb имеет совершенно другой подход к расчету физики в сравнении с оригинальным движком, обходя многие из существующих ограничений. Мы используем генератор рельефа для создания оптимизированной сетки столкновений для каждой комнаты из так называемой «флордаты».
* OpenTomb поддерживает переменную частоту кадров, не ограничиваясь 30fps, как оригинальный движок.
* В OpenTomb применяются распространенные и гибкие библиотеки, такие как OpenGL, OpenAL, SDL и BulletPhysics.
* В OpenTomb применяется скриптовый движок LUA для настроек движка, объектов и описания поведения объектов игрового мира.
...
### Установка ###
Чтобы запустить любой из уровней из исходных игр, вам понадобятся оригинальные ресурсы из соответствующей игры. Эти ресурсы часто имеют тенденцию находится в загадочных форматах, вариаций в играх. Из-за этого вам нужно будет конвертировать некоторые игровые ресурсы к используемым форматам самостоятельно или получить их откуда-то в Сети.
...
Отдельная благодарность: Ado Croft (обширное тестирование),
