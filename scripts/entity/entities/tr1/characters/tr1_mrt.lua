function tr1_mrt_init(id)
    basic_init(id);

    setCharacterParam(id, PARAM_HEALTH, TR1_MRT, TR1_MRT);
    ...
    if(getEntityTypeFlag(id, ENTITY_TYPE_SPAWNED) ~= 0) then
        entity_funcs[id].onSave = function()
            return "tr1_mrt_init(" .. id .. ");\n";
        end;
    end;

    entity_funcs[id].onShoot = function(object_id, activator_id)
        print("entity_functions_enemies->tr1_mrt->onShoot unimplemented ! (lign: 23)");
    end;
