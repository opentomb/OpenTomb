function tr1_crocodile_water_init(id)
    basic_init(id);
	
    setCharacterParam(id, PARAM_HEALTH, TR1_CROCODILE, TR1_CROCODILE);
    setCharacterStateControlFunctions(id, STATE_FUNCTIONS_CROCODILE);
    setCharacterKeyAnim(id, ANIM_TYPE_BASE, ANIMATION_KEY_INIT);
	setEntityMoveType(id, MOVE_UNDERWATER);
    ...
    setCharacterMoveSizes(id, 256.0, nil, nil, nil, nil); -- height, min_step_up_height, max_step_up_height, max_climb_height, fall_down_height
    ...
    if(getEntityTypeFlag(id, ENTITY_TYPE_SPAWNED) ~= 0) then
        entity_funcs[id].onSave = function()
            return "crocodile_water_init(" .. id .. ");\n";
        end;
    end;
    ...
    entity_funcs[id].onAttack = function(object_id, activator_id)
        ...
        if(bone == 8 or bone == 9) then
            setCharacterParam(object_id, PARAM_HIT_DAMAGE, TR1_CROCODILE_DMG * frame_time, TR1_CROCODILE_DMG * frame_time);
