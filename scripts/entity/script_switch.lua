...
function switch_activate(object_id, actor_id)   -- actor ID is needed to activate animation response.
        ...
        function()
            ...
            if(f >= c - 1) then     -- check the end of animation
                if(key ~= nil) then
                    ...
                    setEntityActivityLock(object_id, 1);        -- lock filled hole
                else
    ...
    elseif(off.ready_anim == t and getEntityActivityLock(object_id) ~= 1) then  -- Locked switches doesn't flip back!
