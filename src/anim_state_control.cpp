...
void ent_set_on_floor(entity_p ent, ss_animation_p ss_anim, int state)
{
    if(state == 0x02)
    {
        ...
        ent->transform[12 + 2] = ent->character->height_info.floor_point[2];
...
void ent_to_monkey_swing(entity_p ent, ss_animation_p ss_anim, int state)
{
    if(state == 0x02)
    {
        ...
        ent->transform[12 + 2] = ent->character->height_info.ceiling_point[2] - ent->bf.bb_max[2];
        Entity_GhostUpdate(ent);
        ss_anim->onFrame = NULL;
    }
}
...
int State_Control_Lara(struct entity_s *ent, struct ss_animation_s *ss_anim)
{
    ...
    next_fc.cb = ent->physics.ray_cb;
    next_fc.ccb = ent->physics.convex_cb;
    ent->physics.no_fix_skeletal_parts = 0x00000000;
 ...
 */
    switch(ss_anim->last_state)
    {
        ...
        case TR_STATE_LARA_STOP:
            ...
            {
                ent->physics.no_fix_skeletal_parts = BODY_PART_LEGS_2 | BODY_PART_LEGS_3;
            }
        ...
        case TR_STATE_LARA_RUN_FORWARD:
            ...
            if(ent->move_type == MOVE_ON_FLOOR)
            {
                ent->physics.no_fix_skeletal_parts = BODY_PART_HANDS | BODY_PART_LEGS;;
            }
        ...
        case TR_STATE_LARA_SPRINT:
            ...
            if(ent->move_type == MOVE_ON_FLOOR)
            {
                ent->physics.no_fix_skeletal_parts = BODY_PART_LEGS_1 | BODY_PART_LEGS_2 | BODY_PART_LEGS_3;
            }
        ...
        case TR_STATE_LARA_WALK_BACK:
            ...
            {
                if(ent->physics.no_fix_all == 0x00)
                {
        ...
        case TR_STATE_LARA_UNDERWATER_FORWARD:
            ...
            else if(curr_fc->floor_hit && curr_fc->water && (curr_fc->transition_level - curr_fc->floor_point[2] <= ent->character->max_step_up_height))
            {
                ...
                ss_anim->next_state = TR_STATE_LARA_STOP;
                vec3_copy(ent->character->climb.point, curr_fc->floor_point);   ///@FIXME: without it Lara are pulled high up, but this string was not been here.
                ent->move_type = MOVE_ON_FLOOR;
            }
            else if(cmd->roll)
            {
        ...
        case TR_STATE_LARA_CROUCH_IDLE:
            ent->dir_flag = ENT_MOVE_FORWARD;
            ent->physics.no_fix_skeletal_parts = BODY_PART_HANDS_2 | BODY_PART_HANDS_3 | BODY_PART_LEGS_3;
            move[0] = pos[0];
            move[1] = pos[1];
            ...
            else if(cmd->move[0] == -1)
            {
                ...
                if((Entity_CheckNextPenetration(ent, move) == 0) || (resp->horizontal_collide == 0x00))
                {
                       ...
                       (next_fc.floor_point[2] < pos[2] - ent->character->Height))
                    {
                        ...
                        vec3_copy(curr_fc->floor_normale, next_fc.floor_normale);
                        curr_fc->floor_obj = next_fc.floor_obj;
                        curr_fc->ceiling_hit = next_fc.ceiling_hit;
                        vec3_copy(curr_fc->ceiling_point, next_fc.ceiling_point);
                        vec3_copy(curr_fc->ceiling_normale, next_fc.ceiling_normale);
    ...
    switch(ss_anim->current_animation)
    {
        case TR_ANIMATION_LARA_STAY_JUMP_SIDES:
            ent->physics.no_fix_skeletal_parts |= BODY_PART_HEAD;
            break;
