...
void Character::state_func()
{
    ...
    switch(m_bf.animations.last_state)
    {
        ...
        case TR_STATE_LARA_STOP:
            ...
            {
                if(m_heightInfo.quicksand == QuicksandPosition::None && m_bf.animations.current_animation != TR_ANIMATION_LARA_CLIMB_2CLICK)
                {
                    m_dirFlag = ENT_MOVE_FORWARD;
            ...
            else if(m_command.move[0] == 1)
            {
                if(m_command.shift)
                {
                       ...
                       (next_fc.floor_hit && (next_fc.floor_point[2] > m_transform.getOrigin()[2] - m_maxStepUpHeight) && (next_fc.floor_point[2] <= m_transform.getOrigin()[2] + m_maxStepUpHeight)))
                    {
                        m_moveType = world::MoveType::OnFloor;
                        m_dirFlag = ENT_MOVE_FORWARD;
                        if((m_heightInfo.water || m_heightInfo.quicksand != QuicksandPosition::None) && m_heightInfo.floor_hit && (m_heightInfo.transition_level - m_heightInfo.floor_point[2] > m_wadeDepth))
                        {
                    ...
                    (m_bf.animations.current_animation == TR_ANIMATION_LARA_WALL_SMASH_RIGHT)))
                {
                       ...
                       (next_fc.floor_normale[2] >= m_criticalSlantZComponent)) // trying to climb on
                    {
                        if(m_transform.getOrigin()[2] + 640.0 >= next_fc.floor_point[2])
                        {
                            ...
                            m_transform.getOrigin()[2] = next_fc.floor_point[2] - 512.0f;
                        ...
                        {
                            m_angles[0] = m_climb.edge_z_ang;
                            m_transform.getOrigin()[2] = next_fc.floor_point[2] - 768.0f;
                            m_climb.point = next_fc.floor_point;
                    ...
                    if(m_climb.edge_hit &&
                       (m_climb.next_z_space >= m_height - engine::LaraHangVerticalEpsilon) &&
                       (m_transform.getOrigin()[2] + m_maxStepUpHeight < next_fc.floor_point[2]) &&
                       (m_transform.getOrigin()[2] + 2944.0 >= next_fc.floor_point[2]))  // Trying to climb on
                    {
        ...
        case TR_STATE_LARA_JUMP_BACK:
            ...
            else if((m_moveType == world::MoveType::Underwater) || (m_speed[2] <= -FREE_FALL_SPEED_2))
            {
        ...
        case TR_STATE_LARA_TURN_RIGHT_SLOW:
            ...
            {
                world::Substance substance_state = getSubstanceState();
        ...
        case TR_STATE_LARA_SPRINT:
            ...
            else if(m_moveType == world::MoveType::FreeFalling)
            {
                setAnimation(TR_ANIMATION_LARA_FREE_FALL_FORWARD, 0);
            }
            else if(m_response.killed)
        ...
        case TR_STATE_LARA_WALK_FORWARD:
            ...
            else if((m_heightInfo.water || m_heightInfo.quicksand != QuicksandPosition::None) && m_heightInfo.floor_hit && (m_heightInfo.transition_level - m_heightInfo.floor_point[2] > m_wadeDepth))
            {
                m_bf.animations.next_state = TR_STATE_LARA_WADE_FORWARD;
            }
        ...
        case TR_STATE_LARA_WADE_FORWARD:
            ...
            if(!m_heightInfo.floor_hit || m_moveType == world::MoveType::FreeFalling)  // Free fall, then swim
            {
                setAnimation(TR_ANIMATION_LARA_START_FREE_FALL, 0);
            }
        ...
        case TR_STATE_LARA_WALK_LEFT:
            ...
            else if(m_command.move[1] == -1 && m_command.shift)
            {
                ...
                if(next_fc.floor_hit && (next_fc.floor_point[2] > m_transform.getOrigin()[2] - m_maxStepUpHeight) && (next_fc.floor_point[2] <= m_transform.getOrigin()[2] + m_maxStepUpHeight))
                {
                    if(!m_heightInfo.water || (m_heightInfo.floor_point[2] + m_height > m_heightInfo.transition_level)) // if (floor_hit == 0) then we went to MoveType::FreeFalling.
                    {
                        // continue walking
                    }
                    else
                    {
                        ...
                        m_bf.animations.onFrame = engine::ent_to_on_water;
                    }
                }
                else
                {
        ...
        case TR_STATE_LARA_JUMP_UP:
            ...
            else if(m_command.action && (m_moveType == world::MoveType::Climbing))
            {
                m_bf.animations.next_state = TR_STATE_LARA_HANG;
                setAnimation(TR_ANIMATION_LARA_HANG_IDLE, -1);
            }
            else if((m_response.vertical_collide & 0x01) || (m_moveType == world::MoveType::OnFloor))
            {
        ...
        case TR_STATE_LARA_REACH:
            ...
            if(m_command.action && (m_moveType == world::MoveType::FreeFalling))
            {
                t = engine::LaraTryHangWallOffset + engine::LaraHangWallDistance;
                global_offset = m_transform.getBasis().getColumn(1) * t;
                global_offset[2] += m_bf.bb_max[2] + engine::LaraHangVerticalEpsilon + engine::engine_frame_time * m_speed[2];
                ...
                if((m_dirFlag == ENT_MOVE_BACKWARD) && (m_moveType == world::MoveType::Climbing))
                {
                    m_transform.getOrigin()[0] = m_climb.point[0] - m_transform.getBasis().getColumn(1)[0] * (m_forwardSize + 16.0f);
                    m_transform.getOrigin()[1] = m_climb.point[1] - m_transform.getBasis().getColumn(1)[1] * (m_forwardSize + 16.0f);
                }
            }
            ...
            if(((m_response.vertical_collide & 0x01) || (m_moveType == world::MoveType::OnFloor)) && (!m_command.action || !m_climb.can_hang))
            {
            ...
            if(m_moveType == world::MoveType::Climbing)
            {
                m_speed.setZero();
                m_bf.animations.next_state = TR_STATE_LARA_HANG;
                m_bf.animations.onFrame = engine::ent_to_edge_climb;
        ...
        case TR_STATE_LARA_UNDERWATER_STOP:
            if(m_moveType != world::MoveType::Underwater && m_moveType != world::MoveType::OnWater)
            {
                setAnimation(0, 0);
            }
            else if(m_response.killed)
            ...
            else if(m_moveType == world::MoveType::OnWater)
            {
                m_inertiaLinear = 0.0;
        ...
        case TR_STATE_LARA_ONWATER_FORWARD:
            ...
            {
                t = m_transform.getOrigin()[2];
                Character::getHeightInfo(m_transform.getOrigin(), &next_fc);
                m_transform.getOrigin()[2] = t;
                m_bf.animations.next_state = TR_STATE_LARA_UNDERWATER_FORWARD;
                m_bf.animations.onFrame = engine::ent_set_underwater;                          // dive
            }
            else if(m_command.move[0] == 1)
            {
                if(m_command.action)
                {
                    if(m_moveType != world::MoveType::Climbing)
                    {
                        ...
                        if(m_climb.edge_hit && (m_climb.next_z_space >= m_height - engine::LaraHangVerticalEpsilon))// && (climb->edge_point[2] - pos[2] < ent->max_step_up_height))   // max_step_up_height is not correct value here
                        {
                            m_dirFlag = ENT_STAY;
                            m_moveType = world::MoveType::Climbing;
                            m_bt.no_fix_all = true;
                ...
                {
                    m_bf.animations.next_state = TR_STATE_LARA_WADE_FORWARD;
                    m_bf.animations.onFrame = engine::ent_set_on_floor;                        // to wade
                }
            }
            else
            {
            ...
            if(!m_command.jump)
            {
                if(m_command.move[1] == -1 && m_command.shift)
                {
                    ...
                    else
                    {
                        // walk left
                        m_bf.animations.next_state = TR_STATE_LARA_WALK_LEFT;
                        m_bf.animations.onFrame = engine::ent_set_on_floor;
                    }
                }
                else
                {
        ...
        case TR_STATE_LARA_CRAWL_IDLE:
            ...
            else if(m_command.move[0] == -1)
            {
                ...
                if((checkNextPenetration(move) == 0) || (m_response.horizontal_collide == 0x00))
                {
                    ...
                    {
                        auto temp = m_transform.getOrigin();                                       // save entity position
        ...
        case TR_STATE_LARA_CRAWL_TO_CLIMB:
            m_bt.no_fix_all = true;
            m_bf.animations.onFrame = engine::ent_crawl_to_climb;
            break;
    ...
    switch(m_bf.animations.current_animation)
    {
        ...
        case TR_ANIMATION_LARA_FLY_FORWARD_TRY_HANG:
            if((m_moveType == world::MoveType::FreeFalling) && m_command.action &&
