...
int State_Control_Lara(Character* character, SSAnimation *ss_anim)
{
    int i;
    ...
    bool low_vertical_space = (curr_fc->floor_hit && curr_fc->ceiling_hit && (curr_fc->ceiling_point[2] - curr_fc->floor_point[2] < character->m_height - engine::LaraHangVerticalEpsilon));
    ...
    switch(ss_anim->last_state)
    {
        ...
        case TR_STATE_LARA_STOP:
            ...
            if((climb->can_hang &&
                (climb->next_z_space >= character->m_height - engine::LaraHangVerticalEpsilon) &&
            ...
            else if(cmd->action && character->findTraverse())
            {
                ...
                {
                    t = -character->m_traversedObject->m_bf.boundingBox.min[0] + 72.0f;
                }
                else if(character->m_transform.getBasis().getColumn(1).x() < -0.9)
                {
                    t = character->m_traversedObject->m_bf.boundingBox.max[0] + 72.0f;
                }
                else if(character->m_transform.getBasis().getColumn(1).y() > 0.9)
                {
                    t = -character->m_traversedObject->m_bf.boundingBox.min[1] + 72.0f;
                }
                else if(character->m_transform.getBasis().getColumn(1).y() < -0.9)
                {
                    t = character->m_traversedObject->m_bf.boundingBox.max[1] + 72.0f;
                }
                else
                {
            ...
            else if(cmd->move[0] == 1)
            {
                if(cmd->shift)
                {
                    move = character->m_transform.getBasis().getColumn(1) * engine::PenetrationTestOffset;
                    global_offset = character->m_transform.getBasis().getColumn(1) * engine::WalkForwardOffset;
                    global_offset[2] += character->m_bf.boundingBox.max[2];
                    global_offset += pos;
                ...
                {
                    move = character->m_transform.getBasis().getColumn(1) * engine::PenetrationTestOffset;
                    global_offset = character->m_transform.getBasis().getColumn(1) * engine::RunForwardOffset;
                    global_offset[2] += character->m_bf.boundingBox.max[2];
                ...
                {
                    t = character->m_forwardSize + engine::LaraTryHangWallOffset;
            ...
            else if(cmd->move[0] == -1)
            {
                if(cmd->shift)
                {
                    ...
                    if((character->checkNextPenetration(move) == 0) || (character->m_response.horizontal_collide == 0x00))
                    {
                        global_offset = character->m_transform.getBasis().getColumn(1) * -engine::WalkBackOffset;
                        global_offset[2] += character->m_bf.boundingBox.max[2];
                        global_offset += pos;
                        Character::getHeightInfo(global_offset, &next_fc);
            ...
            else if(cmd->move[1] == 1)
            {
                if(cmd->shift)
                {
                    move = character->m_transform.getBasis().getColumn(0) * engine::PenetrationTestOffset;
                    if((character->checkNextPenetration(move) == 0) || (character->m_response.horizontal_collide == 0x00))
                    {
                        global_offset = character->m_transform.getBasis().getColumn(0) * engine::RunForwardOffset;
                        global_offset[2] += character->m_bf.boundingBox.max[2];
        ...
        case TR_STATE_LARA_WALK_LEFT:
            ...
            {
                global_offset = character->m_transform.getBasis().getColumn(0) * -engine::RunForwardOffset;  // not an error - RUN_... more correct here
                global_offset[2] += character->m_bf.boundingBox.max[2];
                global_offset += pos;
                Character::getHeightInfo(global_offset, &next_fc);
                if(next_fc.floor_hit && (next_fc.floor_point[2] > pos[2] - character->m_maxStepUpHeight) && (next_fc.floor_point[2] <= pos[2] + character->m_maxStepUpHeight))
        ...
        case TR_STATE_LARA_SLIDE_FORWARD:
            ...
            {
                if((cmd->move[0] == 1) && (engine::engine_world.engineVersion >= loader::Engine::TR3))
                {
                    ss_anim->next_state = TR_STATE_LARA_RUN_FORWARD;
                }
                else
        ...
        case TR_STATE_LARA_PUSHABLE_GRAB:
            ...
            if(cmd->action)  //If Lara is grabbing the block
            {
                ...
                character->m_dirFlag = ENT_STAY;
                ss_anim->mode = animation::SSAnimationMode::LoopLastFrame;  //We hold it (loop last frame)
            ...
            {
                character->m_dirFlag = ENT_STAY;
                ss_anim->mode = animation::SSAnimationMode::NormalControl;  // We're no longer looping last frame
        ...
        case TR_STATE_LARA_PUSHABLE_PUSH:
            ...
            if((character->m_traversedObject != nullptr) && (ss_anim->current_frame > 16) && (ss_anim->current_frame < i - 16)) ///@FIXME: magick 16
            {
                ...
                {
                    t = character->m_transform.getOrigin()[0] + (character->m_bf.boundingBox.max[1] - character->m_traversedObject->m_bf.boundingBox.min[0] - 32.0f);
                ...
                {
                    t = character->m_transform.getOrigin()[0] - (character->m_bf.boundingBox.max[1] + character->m_traversedObject->m_bf.boundingBox.max[0] - 32.0f);
                    if(t < character->m_traversedObject->m_transform.getOrigin()[0])
                    {
                        character->m_traversedObject->m_transform.getOrigin()[0] = t;
                        was_traversed = true;
                ...
                {
                    t = character->m_transform.getOrigin()[1] + (character->m_bf.boundingBox.max[1] - character->m_traversedObject->m_bf.boundingBox.min[1] - 32.0f);
                ...
                {
                    t = character->m_transform.getOrigin()[1] - (character->m_bf.boundingBox.max[1] + character->m_traversedObject->m_bf.boundingBox.max[1] - 32.0f);
                    if(t < character->m_traversedObject->m_transform.getOrigin()[1])
                    {
                        character->m_traversedObject->m_transform.getOrigin()[1] = t;
                        was_traversed = true;
        ...
        case TR_STATE_LARA_JUMP_UP:
            ...
            if(cmd->action && (character->m_moveType != MoveType::WallsClimb) && (character->m_moveType != MoveType::Climbing))
            {
                ...
                global_offset = character->m_transform.getBasis().getColumn(1) * t;
                global_offset[2] += character->m_bf.boundingBox.max[2] + engine::LaraHangVerticalEpsilon + engine::engine_frame_time * character->m_speed[2];
                *climb = character->checkClimbability(global_offset, &next_fc, 0.0);
                if(climb->edge_hit)
                {
                    ...
                    pos[0] = climb->point[0] - (engine::LaraHangWallDistance)* character->m_transform.getBasis().getColumn(1)[0];
                    pos[1] = climb->point[1] - (engine::LaraHangWallDistance)* character->m_transform.getBasis().getColumn(1)[1];
                    pos[2] = climb->point[2] - character->m_bf.boundingBox.max[2] + engine::LaraHangVerticalOffset;
                }
                else
                {
                    *climb = character->checkWallsClimbability();
                       ...
                       (character->m_speed[2] < 0.0)) // Only hang if speed is lower than zero.
                    {
                        ...
                        character->m_transform.getOrigin()[2] = std::floor(character->m_transform.getOrigin()[2] / MeteringStep) * MeteringStep;
            ...
            else if(cmd->action && curr_fc->ceiling_climb && (curr_fc->ceiling_hit) && (pos[2] + character->m_bf.boundingBox.max[2] > curr_fc->ceiling_point[2] - 64.0))
            {
        ...
        case TR_STATE_LARA_REACH:
            ...
            if(((character->m_moveType != MoveType::OnFloor)) && cmd->action && curr_fc->ceiling_climb && (curr_fc->ceiling_hit) && (pos[2] + character->m_bf.boundingBox.max[2] > curr_fc->ceiling_point[2] - 64.0))
            {
                ss_anim->next_state = TR_STATE_LARA_MONKEYSWING_IDLE;
                ss_anim->onFrame = ent_to_monkey_swing;
                break;
        ...
        case TR_STATE_LARA_HANG:
            ...
            if(character->m_moveType == MoveType::WallsClimb)
            {
                if(cmd->action)
                {
                    ...
                    {
                        ss_anim->mode = animation::SSAnimationMode::LoopLastFrame;  // Disable shake
                    }
                }
                else
                {
            ...
            if(!resp->killed && cmd->action)  // We have to update climb point every time so entity can move
            {
                t = engine::LaraTryHangWallOffset + engine::LaraHangWallDistance;
                global_offset = character->m_transform.getBasis().getColumn(1) * t;
                global_offset[2] += character->m_bf.boundingBox.max[2] + engine::LaraHangVerticalEpsilon;
                *climb = character->checkClimbability(global_offset, &next_fc, 0.0);
                if(climb->can_hang)
                {
                    climb->point = climb->edge_point;
            ...
            if(character->m_moveType == MoveType::Climbing)
            {
                if(cmd->move[0] == 1)
                {
                    if(climb->edge_hit && (climb->next_z_space >= 512.0) && ((climb->next_z_space < character->m_height - engine::LaraHangVerticalEpsilon) || cmd->crouch))
                    {
                        climb->point = climb->edge_point;
                        ss_anim->next_state = TR_STATE_LARA_CLIMB_TO_CRAWL;  // Crawlspace climb
                    }
                    else if(climb->edge_hit && (climb->next_z_space >= character->m_height - engine::LaraHangVerticalEpsilon))
                    {
                        engine::Sys_DebugLog(LOG_FILENAME, "Zspace = %f", climb->next_z_space);
                        climb->point = climb->edge_point;
        ...
        case TR_STATE_LARA_LADDER_IDLE:
            ...
            else if(cmd->move[0] == 1)
            {
                ...
                *climb = character->checkClimbability(global_offset, &next_fc, 0.0);
                ...
                else if((!curr_fc->ceiling_hit) || (pos[2] + character->m_bf.boundingBox.max[2] < curr_fc->ceiling_point[2]))
                {
        ...
        case TR_STATE_LARA_LADDER_UP:
            ...
            if(cmd->action && character->m_climb.wall_hit != ClimbType::None)
            {
                ...
                *climb = character->checkClimbability(global_offset, &next_fc, 0.0);
                ...
                else if((cmd->move[0] <= 0) && (curr_fc->ceiling_hit || (pos[2] + character->m_bf.boundingBox.max[2] >= curr_fc->ceiling_point[2])))
                {
                    ss_anim->next_state = TR_STATE_LARA_LADDER_IDLE;
                }

                if(curr_fc->ceiling_hit && (pos[2] + character->m_bf.boundingBox.max[2] > curr_fc->ceiling_point[2]))
                {
                    pos[2] = curr_fc->ceiling_point[2] - character->m_bf.boundingBox.max[2];
                }
            }
            else
            {
            ...
            else if(cmd->move[0] == 1)
            {
                if(cmd->action)
                {
                    if(character->m_moveType != MoveType::Climbing)
                    {
                        ...
                        *climb = character->checkClimbability(global_offset, &next_fc, 0.0);
                        ...
                        else
                        {
                            ...
                            global_offset[2] += character->m_maxStepUpHeight + engine::LaraHangVerticalEpsilon;
                            *climb = character->checkClimbability(global_offset, &next_fc, 0.0);
                        }

                        if(climb->edge_hit && (climb->next_z_space >= character->m_height - engine::LaraHangVerticalEpsilon))// && (climb->edge_point[2] - pos[2] < ent->character->max_step_up_height))   // max_step_up_height is not correct value here
                        {
                            character->m_dirFlag = ENT_STAY;
                            character->m_moveType = MoveType::Climbing;
                            character->m_bt.no_fix_all = true;
                    ...
                    if(character->m_moveType == MoveType::Climbing)
                    {
                        ...
                        ent_climb_out_of_water(character, ss_anim, world::animation::AnimUpdate::NewAnim);
                    }
                }
        ...
        case TR_STATE_LARA_CROUCH_IDLE:
            ...
            move[0] = pos[0];
            move[1] = pos[1];
            move[2] = pos[2] + 0.5f * (character->m_bf.boundingBox.max[2] - character->m_bf.boundingBox.min[2]);
        ...
        case TR_STATE_LARA_CRAWL_IDLE:
            ...
            else if(cmd->move[0] == 1)
            {
                ...
                {
                    global_offset = character->m_transform.getBasis().getColumn(1) * engine::CrawlForwardOffset;
                    global_offset[2] += 0.5f * (character->m_bf.boundingBox.max[2] + character->m_bf.boundingBox.min[2]);
                    global_offset += pos;
                    Character::getHeightInfo(global_offset, &next_fc);
            ...
            else if(cmd->move[0] == -1)
            {
                ...
                if((character->checkNextPenetration(move) == 0) || (resp->horizontal_collide == 0x00))
                {
                            ...
                            (next_fc.floor_point[2] < pos[2] - character->m_height))
                    {
                        ...
                        global_offset = character->m_transform.getBasis().getColumn(1) * 0.5 * engine::CrawlForwardOffset;
                        ...
                        *climb = character->checkClimbability(global_offset, &next_fc, 1.5f * character->m_bf.boundingBox.max[2]);
        ...
        case TR_STATE_LARA_MONKEYSWING_IDLE:
            ...
            if(cmd->action && (character->m_moveType != MoveType::Monkeyswing) && curr_fc->ceiling_climb && (curr_fc->ceiling_hit) && (pos[2] + character->m_bf.boundingBox.max[2] > curr_fc->ceiling_point[2] - 96.0))
            {
                character->m_moveType = MoveType::Monkeyswing;
                character->setAnimation(TR_ANIMATION_LARA_MONKEY_IDLE, 0);
                ss_anim->next_state = TR_STATE_LARA_MONKEYSWING_IDLE;
                pos[2] = character->m_heightInfo.ceiling_point[2] - character->m_bf.boundingBox.max[2];
            }
