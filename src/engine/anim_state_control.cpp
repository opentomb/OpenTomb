...
void Character::state_func()
{
    ...
    switch(m_bf.animations.last_state)
    {
        ...
        case TR_STATE_LARA_STOP:
            ...
            {
                engine::engine_world.audioEngine.send(TR_AUDIO_SOUND_LANDING, audio::EmitterType::Entity, id());
        ...
        case TR_STATE_LARA_SLIDE_BACK:
            ...
            engine::engine_world.audioEngine.kill(TR_AUDIO_SOUND_SLIDING, audio::EmitterType::Entity, id());
            break;
        ...
        case TR_STATE_LARA_PUSHABLE_PUSH:
            ...
            if((m_traversedObject != nullptr) && (m_bf.animations.current_frame > 16) && (m_bf.animations.current_frame < i - 16)) ///@FIXME: magick 16
            {
                ...
                if(engine::engine_world.engineVersion > loader::Engine::TR3)
                {
                    if(was_traversed)
                    {
                        if(engine::engine_world.audioEngine.findSource(TR_AUDIO_SOUND_PUSHABLE, audio::EmitterType::Entity, id()) == -1)
                            engine::engine_world.audioEngine.send(TR_AUDIO_SOUND_PUSHABLE, audio::EmitterType::Entity, id());
                    }
                    else
                    {
                        engine::engine_world.audioEngine.kill(TR_AUDIO_SOUND_PUSHABLE, audio::EmitterType::Entity, id());
                    }
                }
                else
                {
        ...
        case TR_STATE_LARA_FREEFALL:
               ...
               (int(m_speed[2]) >= (-FREE_FALL_SPEED_CRITICAL - 100)))
            {
                ...
                engine::engine_world.audioEngine.send(TR_AUDIO_SOUND_LARASCREAM, audio::EmitterType::Entity, id());       // Scream
            }
            ...
            if(m_moveType == MoveType::Underwater)
            {
                ...
                setAnimation(TR_ANIMATION_LARA_FREE_FALL_TO_UNDERWATER, 0);
                engine::engine_world.audioEngine.kill(TR_AUDIO_SOUND_LARASCREAM, audio::EmitterType::Entity, id());       // Stop scream

                // Splash sound is hardcoded, beginning with TR3.
                if(engine::engine_world.engineVersion > loader::Engine::TR2)
                {
                    engine::engine_world.audioEngine.send(TR_AUDIO_SOUND_SPLASH, audio::EmitterType::Entity, id());
                }
            }
            else if((m_response.vertical_collide & 0x01) || (m_moveType == MoveType::OnFloor))
            {
                ...
                {
                    setAnimation(TR_ANIMATION_LARA_STAY_IDLE, 0);
                    engine::engine_world.audioEngine.kill(TR_AUDIO_SOUND_LARASCREAM, audio::EmitterType::Entity, id());
                }
                else if(m_speed[2] <= -FREE_FALL_SPEED_MAXSAFE)
                {
