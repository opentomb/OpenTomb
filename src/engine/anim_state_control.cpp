...
void ent_set_on_floor_after_climb(Character* ent, world::core::SSAnimation* ss_anim, world::AnimUpdate /*state*/)
{
    ...
    if(ss_anim->current_frame >= static_cast<int>(af->frames.size() - 1))
    {
        ...
        world::Entity::updateCurrentBoneFrame(&ent->m_bf, &ent->m_transform);
...
void ent_to_on_water(Character* ent, world::core::SSAnimation* ss_anim, world::AnimUpdate state)
{
    if(state == world::AnimUpdate::NewAnim)
    {
        ...
        ent->ghostUpdate();
        ent->m_moveType = world::MoveType::OnWater;
        ss_anim->onFrame = nullptr;
    }
}
...
void State_Control_Lara(Character* character, world::core::SSAnimation *ss_anim)
{
    int i;
    ...
    bool low_vertical_space = (curr_fc->floor_hit && curr_fc->ceiling_hit && (curr_fc->ceiling_point[2] - curr_fc->floor_point[2] < character->m_height - LaraHangVerticalEpsilon));
    ...
    switch(ss_anim->last_state)
    {
        ...
        case TR_STATE_LARA_STOP:
            ...
            if(character->m_moveType == world::MoveType::OnFloor)
            ...
            if((climb->can_hang &&
                (climb->next_z_space >= character->m_height - LaraHangVerticalEpsilon) &&
                (character->m_moveType == world::MoveType::Climbing)) ||
               (ss_anim->current_animation == TR_ANIMATION_LARA_STAY_SOLID))
            {
                character->m_moveType = world::MoveType::OnFloor;
            }
            ...
            if(character->m_moveType == world::MoveType::FreeFalling)
            {
            ...
            else if(cmd->move[0] == 1)
            {
                if(cmd->shift)
                {
                    move = character->m_transform.getBasis().getColumn(1) * PenetrationTestOffset;
                    global_offset = character->m_transform.getBasis().getColumn(1) * WalkForwardOffset;
                ...
                {
                    move = character->m_transform.getBasis().getColumn(1) * PenetrationTestOffset;
                    global_offset = character->m_transform.getBasis().getColumn(1) * RunForwardOffset;
                    global_offset[2] += character->m_bf.bb_max[2];
                ...
                {
                    t = character->m_forwardSize + LaraTryHangWallOffset;
            ...
            else if(cmd->move[0] == -1)
            {
                if(cmd->shift)
                {
                    ...
                    if((character->checkNextPenetration(move) == 0) || (character->m_response.horizontal_collide == 0x00))
                    {
                        global_offset = character->m_transform.getBasis().getColumn(1) * -WalkBackOffset;
                        global_offset[2] += character->m_bf.bb_max[2];
                        global_offset += pos;
                        Character::getHeightInfo(global_offset, &next_fc);
            ...
            else if(cmd->move[1] == 1)
            {
                if(cmd->shift)
                {
                    move = character->m_transform.getBasis().getColumn(0) * PenetrationTestOffset;
                    if((character->checkNextPenetration(move) == 0) || (character->m_response.horizontal_collide == 0x00))
                    {
                        global_offset = character->m_transform.getBasis().getColumn(0) * RunForwardOffset;
                        global_offset[2] += character->m_bf.bb_max[2];
        ...
        case TR_STATE_LARA_JUMP_BACK:
            ...
            if(resp->vertical_collide & 0x01 || character->m_moveType == world::MoveType::OnFloor)
            {
                if(curr_fc->quicksand != QuicksandPosition::None)
                {
            ...
            else if((character->m_moveType == world::MoveType::Underwater) || (character->m_speed[2] <= -FREE_FALL_SPEED_2))
            {
        ...
        case TR_STATE_LARA_TURN_RIGHT_SLOW:
            ...
            if(cmd->move[0] == 1)
            {
                world::Substance substance_state = character->getSubstanceState();
                if((substance_state == world::Substance::None) ||
                   (substance_state == world::Substance::WaterShallow))
                {
                    if(cmd->shift)
                    {
                        ss_anim->next_state = TR_STATE_LARA_WALK_FORWARD;
                    ...
                    ((ss_anim->last_state == TR_STATE_LARA_TURN_RIGHT_SLOW) && (cmd->move[1] == 1)))
            {
                world::Substance substance_state = character->getSubstanceState();
                if(last_frame &&
                   (substance_state != world::Substance::WaterWade) &&
                   (substance_state != world::Substance::QuicksandConsumed) &&
                   (substance_state != world::Substance::QuicksandShallow))
                {
        ...
        case TR_STATE_LARA_SPRINT:
            ...
            else if(character->m_moveType == world::MoveType::FreeFalling)
            {
                character->setAnimation(TR_ANIMATION_LARA_FREE_FALL_FORWARD, 0);
            }
            else if(resp->killed)
        ...
        case TR_STATE_LARA_WADE_FORWARD:
            ...
            if(!curr_fc->floor_hit || character->m_moveType == world::MoveType::FreeFalling)  // Free fall, then swim
            {
                character->setAnimation(TR_ANIMATION_LARA_START_FREE_FALL, 0);
            }
            else if(curr_fc->water)
            {
                ...
                else if(curr_fc->transition_level - curr_fc->floor_point[2] > (character->m_height - character->m_swimDepth))
                {
                    ...
                    else
                    {
                        ...
                        character->m_moveType = world::MoveType::OnWater;
        ...
        case TR_STATE_LARA_WALK_LEFT:
            ...
            {
                global_offset = character->m_transform.getBasis().getColumn(0) * -RunForwardOffset;  // not an error - RUN_... more correct here
                global_offset[2] += character->m_bf.bb_max[2];
                global_offset += pos;
                Character::getHeightInfo(global_offset, &next_fc);
                if(next_fc.floor_hit && (next_fc.floor_point[2] > pos[2] - character->m_maxStepUpHeight) && (next_fc.floor_point[2] <= pos[2] + character->m_maxStepUpHeight))
        ...
        case TR_STATE_LARA_JUMP_UP:
            cmd->rot[0] = 0.0;
            if(cmd->action && (character->m_moveType != world::MoveType::WallsClimb) && (character->m_moveType != world::MoveType::Climbing))
            {
                t = LaraTryHangWallOffset + LaraHangWallDistance;
                global_offset = character->m_transform.getBasis().getColumn(1) * t;
                global_offset[2] += character->m_bf.bb_max[2] + LaraHangVerticalEpsilon + engine_frame_time * character->m_speed[2];
                *climb = character->checkClimbability(global_offset, &next_fc, 0.0);
                if(climb->edge_hit)
                {
                    ...
                    character->m_moveType = world::MoveType::Climbing;                             // hang on
                    character->m_speed.setZero();

                    pos[0] = climb->point[0] - (LaraHangWallDistance)* character->m_transform.getBasis().getColumn(1)[0];
                    pos[1] = climb->point[1] - (LaraHangWallDistance)* character->m_transform.getBasis().getColumn(1)[1];
                    pos[2] = climb->point[2] - character->m_bf.bb_max[2] + LaraHangVerticalOffset;
                }
                else
                {
                    *climb = character->checkWallsClimbability();
            ...
            if(character->m_moveType == world::MoveType::Underwater)
            {
            ...
            else if(cmd->action && (character->m_moveType == world::MoveType::Climbing))
            {
                ss_anim->next_state = TR_STATE_LARA_HANG;
                character->setAnimation(TR_ANIMATION_LARA_HANG_IDLE, -1);
            }
            else if((resp->vertical_collide & 0x01) || (character->m_moveType == world::MoveType::OnFloor))
            {
            ...
            else
            {
                ...
                {
                    character->m_moveType = world::MoveType::FreeFalling;
                    ss_anim->next_state = TR_STATE_LARA_FREEFALL;
                }
                break;
            }
        ...
        case TR_STATE_LARA_REACH:
            ...
            if(cmd->action && (character->m_moveType == world::MoveType::FreeFalling))
            {
                t = LaraTryHangWallOffset + LaraHangWallDistance;
                global_offset = character->m_transform.getBasis().getColumn(1) * t;
                global_offset[2] += character->m_bf.bb_max[2] + LaraHangVerticalEpsilon + engine_frame_time * character->m_speed[2];
                *climb = character->checkClimbability(global_offset, &next_fc, 0.0);
                ...
                if((character->m_dirFlag == ENT_MOVE_BACKWARD) && (character->m_moveType == world::MoveType::Climbing))
                {
            ...
            if(((character->m_moveType != world::MoveType::OnFloor)) && cmd->action && curr_fc->ceiling_climb && (curr_fc->ceiling_hit) && (pos[2] + character->m_bf.bb_max[2] > curr_fc->ceiling_point[2] - 64.0))
            {
                ss_anim->next_state = TR_STATE_LARA_MONKEYSWING_IDLE;
                ss_anim->onFrame = ent_to_monkey_swing;
                break;
            }
            if(((resp->vertical_collide & 0x01) || (character->m_moveType == world::MoveType::OnFloor)) && (!cmd->action || !climb->can_hang))
            {
            ...
            if(character->m_moveType == world::MoveType::Climbing)
            {
                character->m_speed.setZero();
                ss_anim->next_state = TR_STATE_LARA_HANG;
        ...
        case TR_STATE_LARA_HANG:
            ...
            if(character->m_moveType == world::MoveType::WallsClimb)
            {
                if(cmd->action)
                {
            ...
            if(!resp->killed && cmd->action)  // We have to update climb point every time so entity can move
            {
                t = LaraTryHangWallOffset + LaraHangWallDistance;
                global_offset = character->m_transform.getBasis().getColumn(1) * t;
                global_offset[2] += character->m_bf.bb_max[2] + LaraHangVerticalEpsilon;
                *climb = character->checkClimbability(global_offset, &next_fc, 0.0);
                if(climb->can_hang)
                {
                    climb->point = climb->edge_point;
            ...
            if(character->m_moveType == world::MoveType::Climbing)
            {
                if(cmd->move[0] == 1)
                {
                    if(climb->edge_hit && (climb->next_z_space >= 512.0) && ((climb->next_z_space < character->m_height - LaraHangVerticalEpsilon) || cmd->crouch))
                    {
                        climb->point = climb->edge_point;
                        ss_anim->next_state = TR_STATE_LARA_CLIMB_TO_CRAWL;  // Crawlspace climb
                    }
                    else if(climb->edge_hit && (climb->next_z_space >= character->m_height - LaraHangVerticalEpsilon))
                    {
        ...
        case TR_STATE_LARA_FALL_BACKWARD:
            ...
            else if(character->m_moveType == world::MoveType::Underwater)
            {
                character->m_angles[1] = -45.0;
                cmd->rot[1] = 0.0;
                character->updateTransform();
        ...
        case TR_STATE_LARA_UNDERWATER_STOP:
            if(character->m_moveType != world::MoveType::Underwater && character->m_moveType != world::MoveType::OnWater)
            {
            ...
            else if(character->m_moveType == world::MoveType::OnWater)
            {
            ...
            else if(cmd->move[0] == 1)
            {
                if(cmd->action)
                {
                    if(character->m_moveType != world::MoveType::Climbing)
                    {
                        ...
                        *climb = character->checkClimbability(global_offset, &next_fc, 0.0);
                        ...
                        else
                        {
                            ...
                            global_offset[2] += character->m_maxStepUpHeight + LaraHangVerticalEpsilon;
                            *climb = character->checkClimbability(global_offset, &next_fc, 0.0);
                        }

                        if(climb->edge_hit && (climb->next_z_space >= character->m_height - LaraHangVerticalEpsilon))// && (climb->edge_point[2] - pos[2] < ent->character->max_step_up_height))   // max_step_up_height is not correct value here
                        {
                            character->m_dirFlag = ENT_STAY;
                            character->m_moveType = world::MoveType::Climbing;
                            character->m_bt.no_fix_all = true;
                    ...
                    if(character->m_moveType == world::MoveType::Climbing)
                    {
                        ...
                        ent_climb_out_of_water(character, ss_anim, world::AnimUpdate::NewAnim);
                    }
                }
        ...
        case TR_STATE_LARA_CRAWL_IDLE:
            ...
            else if(cmd->move[0] == 1)
            {
                ...
                {
                    global_offset = character->m_transform.getBasis().getColumn(1) * CrawlForwardOffset;
            ...
            else if(cmd->move[0] == -1)
            {
                ...
                if((character->checkNextPenetration(move) == 0) || (resp->horizontal_collide == 0x00))
                {
                            ...
                            (next_fc.floor_point[2] < pos[2] - character->m_height))
                    {
                        ...
                        global_offset = character->m_transform.getBasis().getColumn(1) * 0.5 * CrawlForwardOffset;
        ...
        case TR_STATE_LARA_MONKEYSWING_IDLE:
            ...
            if(cmd->action && (character->m_moveType != world::MoveType::Monkeyswing) && curr_fc->ceiling_climb && (curr_fc->ceiling_hit) && (pos[2] + character->m_bf.bb_max[2] > curr_fc->ceiling_point[2] - 96.0))
            {
                character->m_moveType = world::MoveType::Monkeyswing;
            ...
            if((character->m_moveType != world::MoveType::Monkeyswing) || !cmd->action)
            {
        ...
        default:
            cmd->rot[0] = 0.0;
            if((character->m_moveType == world::MoveType::Monkeyswing) || (character->m_moveType == world::MoveType::WallsClimb))
            {
                if(!cmd->action)
                {
                    character->setAnimation(TR_ANIMATION_LARA_START_FREE_FALL, 0);
    ...
    switch(ss_anim->current_animation)
    {
        ...
        case TR_ANIMATION_LARA_FLY_FORWARD_TRY_HANG:
            if((character->m_moveType == world::MoveType::FreeFalling) && character->m_command.action &&
