...
void Physics_RoomNearCallback(btBroadphasePair& collisionPair, btCollisionDispatcher& dispatcher, const btDispatcherInfo& dispatchInfo)
{
    ...
    bool has_ghosts = (collisionPair.m_pProxy0->m_collisionFilterGroup == COLLISION_GROUP_GHOST) ||
    ...
    if(r0 && r1 && (has_ghosts || (c0->collision_group & c1->collision_mask) && (c1->collision_group & c0->collision_mask)))
    {
        if(Room_IsInNearRoomsList(r0, r1))
        {
            dispatcher.defaultNearCallback(collisionPair, dispatcher, dispatchInfo);
        ...
        case COLLISION_SHAPE_SINGLE_BOX:
            {
                ...
                bt_engine_dynamicsWorld->addRigidBody(physics->bt_body[0]);
            }
            break;
...
void Physics_GenStaticMeshRigidBody(struct static_mesh_s *smesh)
{
    ...
    if(cshape)
    {
        ...
        bt_engine_dynamicsWorld->addRigidBody(smesh->physics_body->bt_body);
...
struct physics_object_s* Physics_GenRoomRigidBody(struct room_s *room, struct room_sector_s *heightmap, uint32_t sectors_count, struct sector_tween_s *tweens, int num_tweens)
{
    ...
    if(cshape)
    {
        ...
        bt_engine_dynamicsWorld->addRigidBody(ret->bt_body);
