...
void ent_correct_diving_angle(entity_p ent, ss_animation_p ss_anim)
{
    if(ss_anim->frame_changing_state >= 0x02)
    {
        ent->transform.angles[1] = (ss_anim->prev_animation == TR_ANIMATION_LARA_FREE_FALL_FISH) ? (-90.0f) : (-45.0f);
...
int StateControl_LaraDoOneHandWeaponFrame(struct entity_s *ent, struct ss_animation_s *ss_anim, float time)
{
    ...
    if (ss_anim->model->animation_count == 4)
    {
        ...
        switch (CurrentWeaponModelToItemID(ss_anim))
        {
            case ITEM_PISTOL:
                weapon = getPistol();
                ent->character->currentAmmunitionInventoryItemId = ITEM_PISTOL_AMMO;
                break;
            case ITEM_AUTOMAGS:
                weapon = getAutomags();
                ent->character->currentAmmunitionInventoryItemId = ITEM_AUTOMAGS_AMMO;
                break;
            case ITEM_DESERTEAGLE:
                weapon = getDesertEagle();
                ent->character->currentAmmunitionInventoryItemId = ITEM_DESERTEAGLE_AMMO;
                break;
            case ITEM_MAGNUMS:
                weapon = getMagnum();
                ent->character->currentAmmunitionInventoryItemId = ITEM_MAGNUM_AMMO;
                break;
            case ITEM_REVOLVER:
                weapon = getRevolver();
                ent->character->currentAmmunitionInventoryItemId = ITEM_REVOLVER_AMMO;
                break;
            case ITEM_UZIS:
                weapon = getUzi();
                ent->character->currentAmmunitionInventoryItemId = ITEM_UZI_AMMO;
                break;
        }
    ...
    if (ss_anim->model->animation_count > 4)
    {
        ...
        *shotgun, rifles, crossbow, harpoon, launchers (2 handed weapons)*/
        ...
        switch (CurrentWeaponModelToItemID(ss_anim))
        {
            case ITEM_SHOTGUN:
                weapon = getShotgun();
                ent->character->currentAmmunitionInventoryItemId = ITEM_SHOTGUN_AMMO; // TODO multiple ammo in TR4
                break;
            case ITEM_MP5:
                weapon = getMP5();
                ent->character->currentAmmunitionInventoryItemId = ITEM_MP5_AMMO;
                break;
            case ITEM_M16:
                weapon = getM16();
                ent->character->currentAmmunitionInventoryItemId = ITEM_M16_AMMO;
                break;
            case ITEM_CROSSBOW:
                weapon = getCrossbowGun();
                ent->character->currentAmmunitionInventoryItemId = ITEM_CROSSBOW_NORMAL_AMMO; // TODO multiple ammo in TR4
                break;
            case ITEM_GRENADEGUN:
                weapon = getGrenadeGun();
                ent->character->currentAmmunitionInventoryItemId = ITEM_GRENADEGUN_AMMO; // TODO multiple ammo in TR4
                break;
            case ITEM_HARPOONGUN:
                weapon = getHarpoonGun(ent->move_type == MOVE_UNDERWATER);
                ent->character->currentAmmunitionInventoryItemId = ITEM_HARPOONGUN_AMMO;
                break;
            case ITEM_ROCKETGUN:
                weapon = getRocketGun();
                ent->character->currentAmmunitionInventoryItemId = ITEM_ROCKETGUN_AMMO;
                break;
            case ITEM_GRAPPLEGUN:
                weapon = getGrapplinGun();
                ent->character->currentAmmunitionInventoryItemId = ITEM_GRAPPLEGUN_AMMO;
                break;
        }
    ...
    if(sm && (ent->bf->bone_tag_count == sm->mesh_count) && (sm->animation_count >= 4))
    {
        ...
        if (ent->character->state.weapon_ready && (!weapon_state || (ent->character->weapon_id_req != ent->character->weapon_id)))
        {
            ...
            else if (anim_th && anim_th->enabled)
            {
                ...
                {
                    if (IS_TR_II(ver) && anim_th->model->id == TR2_MODEL_GRENADEGUN)
                    {
                        setCurrentWeaponExit(ent);
                    }
                    else if (IS_TR_III(ver) && anim_th->model->id == TR3_MODEL_GRENADEGUN)
                    {
                        setCurrentWeaponExit(ent);
                    }
                    else if ((ver >= TR_IV) && anim_th->model->id == TR4C_MODEL_GRENADEGUN)
                    {
                        setCurrentWeaponExit(ent);
                    }
                }
                ...
                {
                    if (IS_TR_II(ver) && anim_th->model->id == TR2_MODEL_HARPOONGUN)
                    {
                        setCurrentWeaponExit(ent);
                    }
                    else if (IS_TR_III(ver) && anim_th->model->id == TR3_MODEL_HARPOONGUN)
                    {
                        setCurrentWeaponExit(ent);
                    }
                }
