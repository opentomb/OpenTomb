...
#include "state_control_Larson.h"
...
void StateControl_LarsonSetKeyAnim(struct entity_s *ent, struct ss_animation_s *ss_anim, int key_anim)
{
    switch(key_anim)
    {
        case ANIMATION_KEY_INIT:
            Anim_SetAnimation(ss_anim, TR_ANIMATION_LARSON_STAY_QUEUED, 0);
            break;

        case ANIMATION_KEY_DEAD:
            Anim_SetAnimation(ss_anim, TR_ANIMATION_LARSON_DEAD, 0);
            break;
    }
}
...
int StateControl_Larson(struct entity_s *ent, struct ss_animation_s *ss_anim)
{
    ...
    {
        case TR_STATE_LARSON_STAY:
            if(state->dead || cmd->action)
            {
                ss_anim->target_state = TR_STATE_LARSON_AIM;
            }
            else if(cmd->move[0] < 0)
            {
                ss_anim->target_state = TR_STATE_LARSON_STAY_QUEUED;
            }
            else if(cmd->move[0] > 0)
            {
                ss_anim->target_state = (cmd->shift) ? (TR_STATE_LARSON_WALK) : (TR_STATE_LARSON_RUN);
            }
            else
            {
                ss_anim->target_state = TR_STATE_LARSON_STAY;
            }
            break;

        case TR_STATE_LARSON_STAY_QUEUED:
        ...
        case TR_STATE_LARSON_WALK:
            ent->dir_flag = ENT_MOVE_FORWARD;
            if(!state->dead && cmd->shift && (cmd->move[0] > 0))
            {
                ss_anim->target_state = TR_STATE_LARSON_WALK;
            }
            else
            {
                ss_anim->target_state = TR_STATE_LARSON_STAY;
        ...
        case TR_STATE_LARSON_RUN:
            ent->dir_flag = ENT_MOVE_FORWARD;
            if(!state->dead && !cmd->shift && (cmd->move[0] > 0))
            {
                ss_anim->target_state = TR_STATE_LARSON_RUN;
            }
            else
            {
                ss_anim->target_state = TR_STATE_LARSON_STAY;
        ...
        case TR_STATE_LARSON_AIM:
            if(state->dead)
            {
                Entity_SetAnimation(ent, ANIM_TYPE_BASE, TR_ANIMATION_LARSON_DEAD, 0, NULL);
            }
            else if(cmd->action)
            {
                ss_anim->target_state = TR_STATE_LARSON_SHOOT;
            }
            else
            {
                ss_anim->target_state = TR_STATE_LARSON_STAY;
