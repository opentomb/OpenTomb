...
void StateControl_MrTSetKeyAnim(struct entity_s *ent, struct ss_animation_s *ss_anim, int key_anim)
{
    switch(key_anim)
    {
        case ANIMATION_KEY_INIT:
            Anim_SetAnimation(ss_anim, TR_ANIMATION_MRT_STAY, 0);
            break;

        case ANIMATION_KEY_DEAD:
            Anim_SetAnimation(ss_anim, TR_ANIMATION_MRT_DEAD, 0);
            break;
    }
}
...
int StateControl_MrT(struct entity_s *ent, struct ss_animation_s *ss_anim)
{
    ...
    switch(current_state)
    {
        ...
        case TR_STATE_MRT_AIM:
            if(state->dead)
            {
                Entity_SetAnimation(ent, ANIM_TYPE_BASE, TR_ANIMATION_MRT_DEAD, 0, NULL);
            }
            else if(cmd->action)
            {
