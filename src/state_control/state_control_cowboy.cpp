...
void StateControl_CowboySetIdleAnim(struct entity_s *ent, int anim_type, int move_type)
{
    ...
    {
        case MOVE_ON_FLOOR:
            Entity_SetAnimation(ent, anim_type, TR_ANIMATION_COWBOY_STAY, 0, NULL);
            break;
    }
}
...
int StateControl_Cowboy(struct entity_s *ent, struct ss_animation_s *ss_anim)
{
    ...
    switch(current_state)
    {
        case TR_STATE_COWBOY_STAY:
            ...
            {
                ss_anim->next_state = TR_STATE_COWBOY_AIM;
            }
            else if(cmd->move[0] > 0)
            {
                ss_anim->next_state = (cmd->shift) ? (TR_STATE_COWBOY_WALK) : (TR_STATE_COWBOY_RUN);
            }
            else
            {
        ...
        case TR_STATE_COWBOY_WALK:
            ...
            {
                ss_anim->next_state = TR_STATE_COWBOY_WALK;
            }
            else
            {
                ss_anim->next_state = TR_STATE_COWBOY_STAY;
        ...
        case TR_STATE_COWBOY_RUN:
            ...
            if(!state->dead && !cmd->shift && (cmd->move[0] > 0))
            {
                ss_anim->next_state = TR_STATE_COWBOY_RUN;
            }
            else
            {
                ss_anim->next_state = TR_STATE_COWBOY_STAY;
        ...
        case TR_STATE_COWBOY_AIM:
            ...
            {
                ss_anim->next_state = TR_STATE_COWBOY_SHOOT;
            }
            else
            {
                ss_anim->next_state = TR_STATE_COWBOY_STAY;
