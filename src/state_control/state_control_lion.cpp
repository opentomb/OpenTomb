...
void StateControl_LionSetKeyAnim(struct entity_s *ent, struct ss_animation_s *ss_anim, int key_anim)
{
    ...
    switch(key_anim)
    {
        case ANIMATION_KEY_INIT:
            Anim_SetAnimation(ss_anim, TR_ANIMATION_LION_STAY, 0);
            break;

        case ANIMATION_KEY_DEAD:
            if(current_state == TR_STATE_LION_STAY)
            {
                Anim_SetAnimation(ss_anim, TR_ANIMATION_LION_DEAD1, 0);
            }
            else
            {
                Anim_SetAnimation(ss_anim, TR_ANIMATION_LION_DEAD2, 0);
            }
            break;
    }
}
...
int StateControl_Lion(struct entity_s *ent, struct ss_animation_s *ss_anim)
{
    ...
    switch(current_state)
    {
        ...
        case TR_STATE_LION_JUMP_ATTACK:
