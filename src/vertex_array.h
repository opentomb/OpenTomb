...
{
    size_t m_index; // The number of the attribute, used to match it to the shader.
    size_t m_size;  // The number of components of the attribute.
    ...
    size_t m_stride;    // The stride of the attribute in bytes. 0 is not allowed.
    size_t m_offset;    // The start offset of the attribute within its vbo in bytes.

    VertexArrayAttribute() = default;

    VertexArrayAttribute(unsigned index, unsigned size, GLenum type,
                         bool normalized, GLuint vbo, size_t stride, size_t offset)
