...
void vec4_GetPlaneEquation(btScalar eq[4], btScalar poly[12])
{
    ...
    v1[0] = poly[1*4+0] - poly[0*4+0];                                          // получаем первый вектор плоскости
    v1[1] = poly[1*4+1] - poly[0*4+1];
    v1[2] = poly[1*4+2] - poly[0*4+2];
 
    v2[0] = poly[0*4+0] - poly[2*4+0];                                          // получаем второй вектор плоскости
    v2[1] = poly[0*4+1] - poly[2*4+1];
    v2[2] = poly[0*4+2] - poly[2*4+2];
    
    eq[0] = v1[1]*v2[2] - v1[2]*v2[1];                                          // получаем вектор нормали к плоскости
    eq[0] = v1[2]*v2[0] - v1[0]*v2[2];
    eq[2] = v1[0]*v2[1] - v1[1]*v2[0];
    
    t = sqrt(eq[0]*eq[0] + eq[1]*eq[1] + eq[2]*eq[2]);                          // Нормируем вектор нормали
    eq[0] /= t;
    eq[1] /= t;
    eq[2] /= t;
    
    eq[3] = -(poly[0]*eq[0] + poly[1]*eq[1] + poly[2]*eq[2]);                   // расстояние до начала координат
}
...
void vec3_GetPlaneEquation(btScalar eq[4], btScalar v0[3], btScalar v1[3], btScalar v2[3])
{
    ...
    vec3_sub(l1, v1, v0);                                                       // получаем первый вектор плоскости
    vec3_sub(l2, v0, v2);                                                       // получаем второй вектор плоскости
    vec3_cross(eq, l1, l2);                                                     // получаем вектор нормали к плоскости
    ...
    eq[3] = -(v0[0]*eq[0] + v0[1]*eq[1] + v0[2]*eq[2]);                         // расстояние до начала координат
}
...
/**
 * Произведение двух матриц преобразования
 */
...
/**
 * Произведение двух матриц, только src1 обратная
 */
...
/**
 * Произведение двух матриц, только src2 обратная
 */
