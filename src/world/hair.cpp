...
bool Hair::create(HairSetup *setup, const Entity& parent_entity)
{
    ...
    if(!model || !model->hasSkinnedBones())
        return false;
    ...
    m_elements.clear();
    m_elements.resize(model->getBoneCount());
    ...
    for(size_t i = 0; i < m_elements.size(); i++)
    {
        ...
        m_elements[i].mesh = model->getSkinnedBone(i).mesh_base;
        ...
        getWorld()->m_engine->m_bullet.dynamicsWorld->addRigidBody(m_elements[i].body.get(), COLLISION_GROUP_CHARACTERS, COLLISION_GROUP_KINEMATIC);
    ...
    for(size_t i = 0; i < m_elements.size(); i++)
    {
        ...
        getWorld()->m_engine->m_bullet.dynamicsWorld->addConstraint(m_joints[curr_joint].get(), true);
...
void Hair::createHairMesh(const SkeletalModel& model)
{
    ...
    for(size_t i = 0; i < model.getBoneCount(); i++)
    {
         ...
         * correctly.
         */
        m_elements[i].position = model.getSkinnedBone(i).position;
        if(i > 0)
        {
        ...
        for(size_t j = 0; j < original->m_vertices.size(); j++)
        {
            ...
            else
            {
                ...
                m_mesh->m_matrixIndices[verticesStart + j].j = std::min(static_cast<int8_t>(i + 2), static_cast<int8_t>(model.getBoneCount()));
            }
...
{
    lua::Value res = world.m_engine->m_scriptEngine["getHairSetup"](hair_entry_index);
    if(!res.is<lua::Table>())
        return;
...
Hair::~Hair()
{
    ...
    for(auto& element : m_elements)
    {
        if(element.body)
        {
            ...
            getWorld()->m_engine->m_bullet.dynamicsWorld->removeRigidBody(element.body.get());
        }
    }
}
} // namespace world
